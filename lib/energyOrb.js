...
export class EnergyOrbRenderer {
  constructor() {
    this.particles = new ParticleSystem(75); // Optimized pool for mobile performance
    this.ringAngle1 = 0;
    this.ringAngle2 = 0;
    this.ringAngle3 = 0;
  ...
  update(dt, isOrbActive, hand1Points = null, hand2Points = null, colorConfig = {}) {
    ...
    if (hasHands) {
      ...
      // Spawn orbiting spark particles around the orb (optimized for mobile)
      this.spawnTimer += dt;
      if (this.spawnTimer > 0.045) {
        this.spawnTimer = 0;
  ...
  draw(ctx, colorConfig = {}) {
    ...
    this.lightningArcs.forEach((arc) => {
      ...
      ctx.lineWidth = arc.width * 2.8;
      ctx.globalAlpha = this.alpha * arc.alpha * 0.45;
      ctx.stroke();
      ...
      ctx.lineWidth = Math.max(1, arc.width * 0.75);

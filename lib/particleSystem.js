...
export class ParticleSystem {
  ...
  /**
   * Draws all active particles with glowing additive blending (optimized for mobile 60 FPS).
  ...
  draw(ctx) {
    for (let i = 0; i < this.maxParticles; i++) {
      ...
      ctx.beginPath();
      ctx.arc(p.x, p.y, Math.max(1, p.size * 0.55), 0, Math.PI * 2);
      ctx.fillStyle = p.glowColor;
      ctx.globalAlpha = Math.min(1.0, p.alpha * 1.2);
      ctx.fill();
    }
  }
